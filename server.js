...
function applyResult(v){
  ...
  if(v.funFact){$('funFact').innerHTML='<b>✨ La storia dietro la domanda</b>'+esc(v.funFact)+(v.funFactSource?'<small style="display:block;margin-top:6px;color:#6b7c76">Fonte: '+esc(v.funFactSource)+'</small>':'');$('funFact').classList.remove('hidden')}else{$('funFact').classList.add('hidden');$('funFact').innerHTML=''}
...
function renderScores(scores){const sorted=(scores||[]).slice().sort((a,b)=>(b.score||0)-(a.score||0)||(b.pos||0)-(a.pos||0)),board=(scores||[]).slice().sort((a,b)=>(b.pos||0)-(a.pos||0)||(b.score||0)-(a.score||0)),progress=p=>'<span>⭐ rimonta '+(p.outsidePodiumStreak||0)+'/'+(p.comebackTarget||4)+' · ⛔ podi '+(p.podiumCount||0)+'</span>';$('scoreboard').innerHTML=sorted.map((p,i)=>'<div class="score-row '+(key(p.name)===key(currentName)?'me':'')+'"><div class="rank-num">'+(i+1)+'</div><div class="rank-main"><b>'+esc(p.name)+'</b>'+progress(p)+'</div><div class="rank-points">'+(p.score||0)+' pt</div></div>').join('');$('boardScoreboard').innerHTML=board.map((p,i)=>'<div class="score-row '+(key(p.name)===key(currentName)?'me':'')+'"><div class="rank-num">'+(i+1)+'</div><div class="rank-main"><b>'+esc(p.name)+'</b>'+progress(p)+'</div><div class="rank-points">cas. '+(p.pos||0)+'</div></div>').join('');return {sorted,board,mine:meIn(sorted)}}
...
function applyInfo(v){
  ...
  if(result||map){
    ...
    if(result?.funFact){$('contextFact').innerHTML='<b>✨ La storia dietro la domanda</b>'+esc(result.funFact)+(result.funFactSource?'<small style="display:block;margin-top:6px;color:#6b7c76">Fonte: '+esc(result.funFactSource)+'</small>':'');$('contextFact').classList.remove('hidden')}else{$('contextFact').classList.add('hidden');$('contextFact').innerHTML=''}
...
const server = http.createServer((req,res)=>{
  ...
  if(pathname==='/health'){res.writeHead(200,{'Content-Type':'application/json; charset=utf-8'});return res.end(JSON.stringify({ok:true,version:'3.0.0',rooms:rooms.size}))}
